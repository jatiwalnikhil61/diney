...
export default function CustomerMenu() {
    ...
    // Returning customer: show tracker first if active order exists
    const [hasActiveOrder, setHasActiveOrder] = useState(null) // null=checking, bool=resolved
    ...
    // Called by CustomerOrderTracker after it resolves orders
    const handleOrdersLoaded = (currentOrder) => {
        const active = !!currentOrder
        setHasActiveOrder(active)
        if (!active) setShowMenu(true)
    }

    const menuItemsMap = {}
    if (menu) {
        menu.categories.forEach(cat => {
            cat.items.forEach(item => { menuItemsMap[item.id] = item })
        })
    }
    ...
    const placeOrder = async () => {
        ...
        try {
            ...
            if (res.data.show_status_page !== false) {
                ...
                setOrderPlaced({ orderId: res.data.order_id, estimatedMinutes: res.data.estimated_minutes })
                setCart([])
                setDrawerOpen(false)
            }
        } catch (err) {
    ...
    if (orderPlaced) {
        return (
            <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 32, textAlign: 'center', background: '#fff' }}>
                <div style={{ fontSize: 64, marginBottom: 16 }}>✅</div>
                <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Order Confirmed!</h1>
                <p style={{ fontSize: 16, color: '#6b7280', marginBottom: 8 }}>Your order has been placed successfully.</p>
                <p style={{ fontSize: 14, color: '#9ca3af', marginBottom: 24 }}>Estimated time: ~{orderPlaced.estimatedMinutes || 10} minutes</p>
                <button onClick={() => setOrderPlaced(null)} style={{ padding: '12px 32px', background: '#111827', color: '#fff', border: 'none', borderRadius: 12, fontSize: 15, fontWeight: 600, cursor: 'pointer', minHeight: 44 }}>
                    Order More
                </button>
            </div>
        )
    ...
    const trackerLoaded = hasActiveOrder !== null
    ...
    return (
        <div className="min-h-screen bg-gray-50 pb-24">
            ...
            {/* Returning customer with active order: tracker-first */}
            {trackerLoaded && hasActiveOrder && !showMenu ? (
                <div style={{ padding: '0 0 24px' }}>
                    ...
                    <div style={{ padding: '0 16px' }}>
                        <button
                            ...
                            style={{ width: '100%', padding: '14px', background: '#111827', color: '#fff', border: 'none', borderRadius: 12, fontSize: 15, fontWeight: 600, cursor: 'pointer', minHeight: 44 }}
                        >
                    ...
                    <div className="sticky top-[85px] z-20 bg-white border-b border-gray-100 px-4 py-2 overflow-x-auto flex gap-2 scrollbar-hide">
                        {menu.categories.map(cat => (
                            <button
                                ...
                                className={`shrink-0 px-4 py-1.5 rounded-full text-sm font-medium transition-colors cursor-pointer ${activeCategory === cat.id ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                            >
                                {cat.name}
                            </button>
                        ))}
                                            ...
                                            <div key={item.id} className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
                                                <div className="flex items-start justify-between gap-3">
                                                    ...
                                                    <div className="shrink-0 flex flex-col items-end gap-2">
                                                        {item.photo_url && (
                                                            <img src={item.photo_url} alt={item.name} className="w-20 h-20 rounded-lg object-cover border border-gray-100" />
                                                        )}
                                                        {qty === 0 ? (
                                                            <button onClick={() => updateCart(item.id, 1)} style={{ minHeight: 36 }} className="px-5 py-1.5 rounded-lg border border-gray-900 text-gray-900 text-sm font-semibold hover:bg-gray-900 hover:text-white transition-colors cursor-pointer">
                                                                Add
                                                            </button>
                                                        ) : (
            ...
            {/* Cart bar — only visible when menu is shown */}
